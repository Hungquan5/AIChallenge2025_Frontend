...
// ============================================================================
// === 1. REUSABLE & ENHANCED CHILD COMPONENTS (No changes needed here) ========
// ============================================================================
...
const FeatureToggleButton = ({ active, icon, onClick, activeColor }: {
    ...
    <button
        className={`${featureToggleButtonClass} ${active ? `${activeColor} ${featureToggleActiveClass}` : featureToggleInactiveClass}`}
        onClick={onClick}
    >
        {icon}
    </button>
...
const EnhancedUploadArea = ({ onImageUpload, imageFile, index }: {
    ...
    return (
        ...
        >
            {imageFile ? (
                <>
                    <img src={URL.createObjectURL(imageFile)} alt="Uploaded preview" className={uploadedImageClass} />
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl flex items-center justify-center">
                        <button onClick={(e) => { e.preventDefault(); onImageUpload({ target: { files: null } }) }} className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white text-xs font-semibold rounded-lg hover:bg-red-700 transition-colors shadow-lg">
...
const SmartLanguageToggle = ({ query, onUpdate, index }: { query: Query; onUpdate: (index: number, updated: Partial<Query>) => void; index: number; }) => {
    ...
    return (
        <button onClick={handleLanguageToggle} disabled={isTranslating} className={`${languageToggleClass} ${isTranslating ? 'opacity-75 cursor-wait' : ''} flex items-center gap-2 justify-center`}>
            {isTranslating ? (
                <><Loader2 className="w-5 h-5 animate-spin" /><span className="text-xs">Translating...</span></>
            ) : (
                query.lang === 'eng' ? <Globe className="w-5 h-5" /> : <Languages className="w-5 h-5" />
            )}
        </button>
    );
};

// ✅ REFACTORED: `icon` prop is now `React.ReactNode` for better type safety and consistency
...
// ============================================================================
// === 2. PROPS INTERFACE (No changes needed) =================================
// ============================================================================
...
// ============================================================================
// === 3. NEW CUSTOM HOOK `useQueryItem` ======================================
// ============================================================================

const useQueryItem = ({ index, query, onUpdate, onSingleSearchResult, user, onInsertAfter, onNext, onPrev }: QueryItemProps) => {
    ...
    // Effect to sync local state when props change from parent
    useEffect(() => {
    ...
    const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        setLocalText(value); // Update local state immediately for responsive UI
    ...
    const handleItemSearch = async () => {
        if (!user) {
            alert("Cannot search without a user session.");
            return;
        }
        setIsItemSearching(true);
        try {
            const { asr, ocr, origin, obj, lang, imageFile, text } = query;
            const apiQuery = { asr, ocr, origin, obj, lang, text, image: '' };

            if (imageFile) {
                apiQuery.image = await fileToBase64(imageFile);
                apiQuery.text = ''; // Ensure text is empty for image search
            }
            
            const results = await searchBySingleQuery(apiQuery, user.username);
    ...
    return {
        queryMode, setQueryMode, showOCR, setShowOCR, showASR, setShowASR, showOBJ, setShowOBJ, isItemSearching, localText,
        handleModeToggle, handleFeatureToggle, handleImageUpload, handleTextChange, handleKeyDown, handleItemSearch
    };
};

// ============================================================================
// === 4. REFACTORED `QueryItem` COMPONENT ====================================
// ============================================================================

const QueryItem: React.FC<QueryItemProps> = (props) => {
    const { index, query, onUpdate, onFocus, onInsertAfter, onRemove, disableRemove, focusRequest, onFocusRequestConsumed, modeChangeRequest, onModeChangeRequestConsumed } = props;
    ...
    // Refs for direct DOM manipulation remain in the component
    ...
    // Effect for handling focus requests from parent (DOM-related)
    useEffect(() => {
        if (focusRequest?.index === index) {
            const { field } = focusRequest;
            const focusMap = {
                ocr: () => { setShowOCR(true); setTimeout(() => ocrInputRef.current?.focus(), 0); },
                asr: () => { setShowASR(true); setTimeout(() => asrInputRef.current?.focus(), 0); },
                obj: () => { setShowOBJ(true); setTimeout(() => objInputRef.current?.focus(), 0); },
            };
            focusMap[field]?.();
    ...
    // Effect for handling mode change requests from parent (state-related but triggered externally)
    useEffect(() => {
        if (modeChangeRequest?.index === index) {
            setQueryMode(modeChangeRequest.mode);
            onModeChangeRequestConsumed();
        }
    }, [modeChangeRequest, index, onModeChangeRequestConsumed, setQueryMode]);
    ...
    return (
        <div className={`${queryItemContainerClass} ${isItemSearching ? 'opacity-75 pointer-events-none' : ''}`}>
            ...
            <div className={bottomControlsClass}>
                ...
                <div className="flex items-center gap-2">
                    {/* ✅ REFACTORED: Passing Lucide icon components directly to ActionButton */}
                    <ActionButton onClick={handleItemSearch} icon={<Search className="w-5 h-5" />} tooltip="Search this item" variant="search" disabled={isItemSearching} />
                    <ActionButton onClick={() => onInsertAfter(index)} icon={<Plus className="w-5 h-5" />} tooltip="Add query after" />
                    {!disableRemove && (
                        <ActionButton onClick={() => onRemove(index)} icon={<X className="w-5 h-5" />} tooltip="Remove query" variant="danger" />
                    )}
                </div>
            </div>
        </div>
